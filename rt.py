...
class RayTracer(object):
    ...
    def rtRayColor(self,intercept,rayDirection,recursion=0):
        ...
        if material.matType == OPAQUE:  
            for light in self.lights:
                if light.lightType=="Ambient":
                    ...
                    if light.lightType=="Directional":
                        ...
                        lightDir = algebra.sub_vec(light.point,intercept.point)
                        lightDir = algebra.normalize_vec(lightDir)
            ...
            reflect = reflectVector(intercept.normal,algebra.negate_vec(rayDirection))
            ...
            outside = algebra.dot_vec(rayDirection,intercept.normal)<0
            bias = algebra.mul_veck(0.001,intercept.normal)

            reflect = reflectVector(intercept.normal,algebra.negate_vec(rayDirection))
            reflectOrig = algebra.add_vec(intercept.point,bias) if outside else algebra.sub_vec(intercept.point,bias)
            ...
            if not totalInternalReflection(intercept.normal,rayDirection,1.0,material.ior):
                ...
                refractOrig = algebra.sub_vec(intercept.point,bias) if outside else algebra.add_vec(intercept.point,bias)
                ...
                reflectColor = algebra.mul_veck(Kr,reflectColor)
                refractColor = algebra.mul_veck(Kt,refractColor)
    ...
    def rtRender(self):
        ...
        for i, j in indeces:
            ...
            if 0<=x<self.width and 0<=y<self.height:
                ...
                direction = algebra.normalize_vec(direction)
